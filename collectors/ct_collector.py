...
- crt.sh는 무료 공개 서비스라 장애가 잦다 (2026-08-25 구현 당일 전면 502 실측)
...
조회량 제한·재시도 정책의 최종 값은 팀 협의 대상 (아키텍처 V2 Provider 조회 원칙).
"""
import json
...
CT_LOOKUP_URL = "https://crt.sh/"
CT_TIMEOUT_SECONDS = 10.0    # crt.sh 무응답 대비 — 스캔 전체를 붙잡아 두지 않는다
CT_MAX_ATTEMPTS = 2          # 일시 오류(502 등) 대비 재시도 횟수. 최종 값은 팀 협의 대상
...
def collect(fingerprint: str | None, sct_timestamps: list[str] | None = None) -> dict:
    ...
    출력: fingerprint / first_seen(ISO UTC | null) / source("embedded_sct"|"crt.sh"|null)
          / sct_count(int | null) / log_entries(int | null) / errors[]
