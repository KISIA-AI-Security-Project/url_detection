...
[목적] 웹페이지 표시가 아니라 '파일 다운로드를 강제'하는 응답인지 확인한다.
...
반면 L2-H-08(Refresh)은 정상 용법이 사실상 없는 헤더라 '존재 자체'가 기준이다.
...
def analyze(raw: dict) -> dict:
    ...
    if cd_value is None:
        # 헤더 자체가 없음 = 일반적인 웹페이지 (정상 사이트 대부분이 이 경우)
        return {
            "id": "L2-H-07",
            "scanner": "header",
            "name": "forced_download",
            "detected": False,
            "evidence": {
                "filename": None,
                "extension": None,
    ...
    # 결정 ①: 기준은 'attachment' 값 (inline은 정상 용법이므로 미탐지 — 모듈 docstring 참고)
    detected = parsed["type"] == "attachment"

    # 결정 ②: 확장자 = 마지막 점 뒤, 소문자 (invoice.pdf.exe → exe). 점이 없으면 null
    ...
    return {
        ...
        "detected": detected,
        "evidence": {
            # 결정 ③: filename 없는 attachment도 강제는 사실 → detected true + filename null
            # ("확인 안 됨"과 "없음"의 구분). inline이어도 filename은 관측 사실이므로 보존.
            "filename": parsed["filename"],
            "extension": extension,
            # 결정 ④: 헤더 원문 전체 보존 — 파싱에서 놓친 것이 있어도
            # 사람/LLM이 재확인할 수 있게 (Raw 보존 원칙의 evidence 내 적용)
