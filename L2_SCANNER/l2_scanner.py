...
[처리 흐름]
    scan(url)
      ...
      2. Certificate Collector가 TLS handshake 1회로 인증서 수집 (collectors/certificate_collector.py)
         — 대상 호스트는 HTTPS URL(최종 도착 우선, 없으면 원본)에서 결정
      3. CT Collector가 CT 최초 관측 시각 수집 (collectors/ct_collector.py)
         — 내장 SCT 우선(접속 없음), SCT 없는 인증서만 crt.sh 폴백 조회
      4. Header Analyzer 8종 + Certificate Analyzer 6종이 Raw Data를 공유해 Signal 생성
         (Analyzer는 네트워크 재접속 없음. C-06만 CT Raw Data를 읽는다)
      5. 통합 명세서 10장 형식의 결과 JSON(dict)으로 조립해 반환
...
def _now_iso() -> str:
    """로컬 타임존이 붙은 ISO 8601 시각 (명세서 예시: 2026-08-24T09:20:10+09:00)"""
...
def _tls_target(raw_http: dict) -> tuple[str | None, int]:
    ...
    우선순위: ① 최종 도착 URL이 https면 그 호스트 — 사용자가 실제로 도달하는 지점의
    인증서가 분석 대상. ② 최종 응답을 못 받았으면 원본 URL이 https일 때 그 호스트.
...
def scan(url: str) -> dict:
    """URL 하나를 관측·분석하고 명세서 10장 형식의 L2 결과를 반환한다."""
    started_at = _now_iso()

    # ① HTTP 수집 (접속 1회)
    raw_http = collect(url)

    # ② TLS 인증서 수집 (handshake 1회) — HTTPS 대상이 없으면 unknown 구조로 남는다
    tls_host, tls_port = _tls_target(raw_http)
    raw_tls = collect_certificate(tls_host, tls_port)

    # ③ CT 최초 관측 수집 — 내장 SCT 우선(접속 없음), 없으면 crt.sh 폴백 조회
    ...
    # ④ 분석 — Analyzer는 네트워크 접속 없이 Raw Data만 읽는다
    signals = [analyzer.analyze(raw_http) for analyzer in HEADER_ANALYZERS]
    signals += [analyzer.analyze(raw_tls) for analyzer in CERTIFICATE_ANALYZERS]
    signals += [ct_first_seen.analyze(raw_ct)]   # C-06만 CT Raw Data를 읽는다
    ...
    return {
        ...
        "target": {
            ...
            "final_url": final_url,
            # 최종 응답을 못 받았으면 unknown(null) — "확인 안 됨"과 "없음"의 구분
        ...
        "raw": {
            ...
            # 명세서 6장 TLS 트리 — Certificate Collector의 관측 결과
            "tls": {
        ...
        # HTTP·TLS·CT 세 곳의 실패·차단 기록을 합쳐 보존
        "errors": raw_http["errors"] + raw_tls["errors"] + raw_ct["errors"],
    }
