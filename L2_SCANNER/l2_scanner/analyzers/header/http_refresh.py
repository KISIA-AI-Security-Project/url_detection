...
[목적] Refresh 헤더를 이용한 자동 페이지 이동 예약을 관측한다.
...
SIGNAL = {"id": "L2-H-08", "scanner": "header", "name": "http_refresh"}
...
def analyze(raw: dict) -> dict:
    ...
    if refresh_value is None:
        # 최종 응답을 아예 못 받았으면 헤더 유무 자체를 관측 못 한 것 -> 판정 불가(null).
        # 응답을 받았는데 헤더가 없으면 미관측 -> false (정상 사이트 대부분이 이 경우)
        return {
            **SIGNAL,
