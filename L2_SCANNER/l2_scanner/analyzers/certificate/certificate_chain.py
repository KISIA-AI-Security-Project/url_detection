...
- chain_valid=false -> 검증 실패 관측. 사유 원문은 error에 (self-signed / expired / unable to get local issuer 등 — 다른 C 신호와의 교차 근거)
- chain_valid=null  -> TLS 자체가 안 됨 = 확인 못 함 (unknown != 비정상)

네트워크 접속 없음 — Certificate Collector가 수집, 파싱한 값을 재사용한다.
"""

SIGNAL = {"id": "L2-C-05", "scanner": "certificate", "name": "certificate_chain"}
...
def analyze(tls: dict) -> dict:
    ...
    return {
        **SIGNAL,
        # 체인 비정상이 관측됨(valid false)이 신호. TLS 자체가 안 돼 확인 못 했으면
        # (valid null) 판정 불가 -> null - unknown != 비정상 
        "detected": None if valid is None else valid is False,
        "evidence": {
            "valid": valid,
