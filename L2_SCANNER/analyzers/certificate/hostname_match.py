...
- 와일드카드 규칙: "*.example.com"은 왼쪽 라벨 '한 칸'만 대체한다.
    a.example.com   ← 일치
...
def analyze(tls: dict) -> dict:
    ...
    return {
        "id": "L2-C-03",
        "scanner": "certificate",
        "name": "hostname_certificate_match",   # 노션 L2-C-03 페이지의 Signal 예시와 동일한 이름
        # '불일치가 관측됨'이 신호 — 대조 못 함(null)은 false (unknown ≠ 불일치)
        "detected": matched is False,
        "evidence": {
            "matched": matched,
            "hostname": hostname,
