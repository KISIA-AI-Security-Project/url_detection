...
[목적] 리다이렉트 목적지가 일반 도메인이 아닌 '직접 IP'인지 확인한다.
...
[범위 판단 — 계층 간 역할 분리]
'원본 URL 자체가 IP인 경우'는 접속 없이 문자열만으로 알 수 있으므로 L0의 담당이다.
L2는 "접속했더니 IP로 보내더라"라는 동적 관측만 담당한다.
...
def _host_ip(url: str):
    ...
    자동으로 제거된 '순수 호스트'가 나와서 IP 판별이 어긋나지 않는다.
    """
...
def analyze(raw: dict) -> dict:
    ...
    # evidence에는 '첫 번째' IP hop만 담는다 — 명세서 8장의 출력 항목이 단수형.
    # (여러 IP 경유 횟수(ip_hop_count) 추가는 팀 협의 대기 중인 제안)
    first = ip_hops[0] if detected else {"destination_url": None, "destination_ip": None}
