...
[입력]  Raw Data의 download{filename, extension, mime_type},
        response_body{detected_type, sha256}
...
[L2-H-07과의 분업]
H-07 = "다운로드를 '강제'하는가"(행위), H-06 = "그 파일이 '위험'한가"(유형).
pdf 강제 다운로드는 H-07 true + H-06 false. 관측 대상이 다른 독립 신호이며,
둘 다 true면 "위험 파일을 강제로 내려꽂는" 강한 조합 — 해석은 Rule/LLM의 몫.

[sha256을 evidence에 담는 이유]
① 분석 시점의 파일을 봉인(악성 파일은 접속마다 바뀌거나 사라짐)
② S3 원본과의 무결성 링크  ③ VirusTotal 등 위협 DB 조회 열쇠
④ 동일 파일의 다중 URL 유포 식별. 단, 바디가 상한에 잘린 경우(truncated)
Collector가 sha256을 null로 남긴다 — 부분 해시는 파일 식별자로 쓰면 거짓 정보.

네트워크 접속 없음 — L2-H-01의 Collector가 수집한 값을 재사용한다.
"""

# 위험 확장자 명단 — '지식 데이터'로 로직과 분리 관리 (H-04 명단과 같은 known-list 방식,
# 확장·관리 주체는 팀 협의 대상)
...
def analyze(raw: dict) -> dict:
    ...
    return {
        ...
        "evidence": {
            ...
            "sha256": body["sha256"],             # truncated 시 null (모듈 docstring 참고)
        },
    }
