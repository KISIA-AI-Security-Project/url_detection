...
이걸 '도메인 변경'으로 세면 정상 사이트가 대량 오탐된다. 그래서 소유 단위
...
def analyze(raw: dict) -> dict:
    ...
    # 이웃한 두 지점을 순서대로 비교해 소유자가 '바뀐 횟수'를 센다.
    ...
    return {
        "id": "L2-H-02",
        "scanner": "header",
        "name": "redirect_domain_change",
        "detected": change_count > 0,   # 변경이 1회 이상 '관측'되었는가 (악성 판정 아님)
        "evidence": {
